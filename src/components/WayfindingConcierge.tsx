...
export const WayfindingConcierge: React.FC = () => {
  ...
  return (
    <div className="panel-glass rounded-xl p-5 flex flex-col h-[500px] relative scanlines">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-card-border pb-3 mb-3 gap-2">
        ...
        <div className="flex items-center space-x-2 text-xs">
          ...
          <button
            ...
            className={`flex items-center space-x-1.5 px-2.5 py-1 rounded border transition-all focus:outline-none focus:ring-1 focus:ring-accent-gold focus:border-accent-gold ${
              accessibilityMode
          ...
          {/* Languages Selector */}
          <div className="bg-stadium-green-dark/40 border border-card-border rounded flex overflow-hidden" role="group" aria-label="Select language">
            {(['en', 'es', 'fr'] as const).map(lang => (
              <button
                key={lang}
                onClick={() => setLanguage(lang)}
                className={`px-2 py-0.5 text-[10px] font-bold uppercase transition-all focus:outline-none focus:ring-1 focus:ring-accent-gold ${
                  language === lang
                    ? 'bg-accent-gold text-stadium-green-dark'
                    : 'text-foreground/75 hover:bg-stadium-green-light/40'
                }`}
                aria-label={`Switch interface and voice synthesizer pronunciation to ${lang === 'en' ? 'English' : lang === 'es' ? 'Spanish' : 'French'}`}
      ...
      {/* Messages Output (Announces new chats automatically via aria-live) */}
      <div 
        className="flex-1 overflow-y-auto pr-1 mb-3 space-y-3 scrollbar"
        aria-live="polite"
      ...
      >
        {messages.map((msg, index) => {
          ...
          return (
            ...
            >
              ...
              >
                {isSystem ? <Sparkles className="w-3.5 h-3.5" aria-hidden="true" /> : <User className="w-3.5 h-3.5" aria-hidden="true" />}
              </div>
              <div
              ...
              >
                ...
                {isSystem && msg.isAccessible && (
                  <div className="bg-accent-gold/20 border border-accent-gold/40 text-accent-gold text-[9px] px-1.5 py-0.5 rounded font-scoreboard w-fit mb-2 flex items-center space-x-1 font-bold">
                    <Accessibility className="w-2.5 h-2.5" aria-hidden="true" />
        ...
        {loading && (
          <div className="flex items-center space-x-2 text-accent-gold/70 text-xs italic pl-2" role="status" aria-live="polite">
      ...
      <div className="flex flex-wrap gap-1.5 pb-3 border-t border-card-border/50 pt-2" role="group" aria-label="Suggested questions">
        {currentSuggestions.map((sug, i) => (
          <button
            key={i}
            onClick={() => handleSendMessage(sug.query)}
            disabled={loading}
            className="text-[10px] bg-stadium-green-dark/40 hover:bg-stadium-green-light/40 border border-stadium-green/30 text-foreground/80 rounded px-2.5 py-1 text-left transition-all truncate max-w-full cursor-pointer hover:border-accent-gold/40 focus:outline-none focus:ring-1 focus:ring-accent-gold focus:border-accent-gold"
      ...
      >
        <input
          ...
          className="flex-1 bg-stadium-green-dark/60 border border-card-border rounded px-3 py-2 text-xs text-foreground placeholder-foreground/45 focus:outline-none focus:ring-1 focus:ring-accent-gold focus:border-accent-gold"
          disabled={loading}
          aria-label="Type your stadium concierge question"
        />
        <button
          type="submit"
          disabled={loading || !inputText.trim()}
          className="bg-accent-gold hover:bg-accent-gold-hover disabled:opacity-40 disabled:hover:bg-accent-gold text-stadium-green-dark p-2 rounded transition-all cursor-pointer focus:outline-none focus:ring-1 focus:ring-accent-gold focus:border-accent-gold"
          aria-label="Send message to stadium concierge"
        >
